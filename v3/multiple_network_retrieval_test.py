...
def run_retrievals_on_networks_measured(networks_in):
    ...
    # supervised_retrieval_obj = supervised_retrieval.SupervisedRetrieval(networks_in[0])
...
def measured_retrieval(networks_in):
    data = run_retrievals_on_networks_measured(networks_in)
...
def run_retrievals_on_networks_simulated(networks_in):
    # use one of the networks to retrieve the measured trace
    measured_trace = get_measured_trace.trace
    # supervised_retrieval_obj = supervised_retrieval.SupervisedRetrieval("MLMRL_noise_resistant_net_angle_18")
    supervised_retrieval_obj = supervised_retrieval.SupervisedRetrieval(networks_in[0])
...
def simulated_retrieval(networks_in):
    data = run_retrievals_on_networks_simulated(networks_in)
    with open("multiple_net_retrieval_test.p", "wb") as file:
        pickle.dump(data, file)
    ...
    # plt.show()
    plt.savefig("./stdev_test_photon_simulated.png")

def noise_test(test_run, net_model):
    ...
    for counts in counts_list:
        ...
        for retrieval_type in ["proof", "autocorrelation", "normal"]:
            ...
            # ----------------------------------------
            # resource error with unsupervised training
            # ----------------------------------------

            # # +++++ run unsupervised learning retrieval+++++
            # unsupervised_retrieval = UnsupervisedRetrieval(
                        # run_name=run_name+"_unsupervised_"+retrieval_type, iterations=0,
                        # retrieval=retrieval_type,
                        # modelname=net_model, measured_trace=measured_trace,
...
class DataSaver():
    ...
    def collect(self, counts, retrieval_type, nn_init, ga):
...
def get_fake_measured_trace(counts, plotting, run_name=None):
    ...
    with tf.Session() as sess:
        ...
        ir_f = sess.run(ir_E_prop["E_prop"]["f_cropped"], feed_dict=feed_dict)[0]
...
class UnsupervisedRetrieval:
    def __init__(self, run_name, iterations, retrieval, modelname, measured_trace,
        ...
        # init data object
        self.get_data = self.network3.GetData(batch_size=10)
    ...
    def update_plots(self):
        ir_f = self.sess.run(self.nn_nodes["general"]["phase_net_output"]["ir_E_prop"]["E_prop"]["f_cropped"],feed_dict=self.feed_dict)[0]
